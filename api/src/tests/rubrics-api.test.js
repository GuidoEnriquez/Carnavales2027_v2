...
test("API ADMIN crea ítems y devuelve especialidades derivadas", { skip: !process.env.TEST_DATABASE_URL }, async (context) => {
  ...
  try { const base=`http://127.0.0.1:${server.address().port}`, headers={'content-type':'application/json','x-test-session':'admin'};
    ...
    const itemResponse=await fetch(`${base}/api/v1/rubrics/${rubric.id}/items`,{method:'POST',headers,body:JSON.stringify({name:'Ítem',code:'ITEM',specialtyId:specialty.id})}); assert.equal(itemResponse.status,201); const item=await itemResponse.json();
    const criterionResponse=await fetch(`${base}/api/v1/rubrics/${rubric.id}/criteria`,{method:'POST',headers,body:JSON.stringify({description:'Descripción reglamentaria',displayOrder:1})}); assert.equal(criterionResponse.status,201); const criterion=await criterionResponse.json();
    ...
    const updatedCriterion=await fetch(`${base}/api/v1/rubric-criteria/${criterion.id}`,{method:'PATCH',headers,body:JSON.stringify({description:'Criterio editado',displayOrder:2,active:false})});assert.equal(updatedCriterion.status,200);assert.equal((await updatedCriterion.json()).description,'Criterio editado');
