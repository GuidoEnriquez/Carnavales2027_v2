...
export async function getReadiness({ client = getPool(), eventId }) {
  ...
  const {rows:incompleteRubrics}=await client.query(`SELECT r.id,r.code FROM rubric r WHERE r.event_id=$1 AND r.active AND NOT EXISTS(SELECT 1 FROM evaluation_item i JOIN event_specialty s ON s.id=i.specialty_id WHERE i.rubric_id=r.id AND i.active AND s.active)`,[eventId]);
