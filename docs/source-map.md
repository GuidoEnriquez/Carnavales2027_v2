...
- Spec 022: tiempo real interno (Fase 4 del Plan Maestro). Server-Sent Events (SSE) en `/api/v1/monitor/stream`, secreto estricto de voto (RF-190), fallback transparente a polling (RF-193), alertas de anomalía operativa (RF-194) y modo "Pared de sala" para proyectores (RF-195). Validada y cerrada el 2026-09-08. Corrección post-commit el 2026-09-10: los eventos de dominio (`RESULTS_RELEASED`, `OFFICIAL_RECORD_EMITTED`, `RESULTS_TIE_BREAKER_CEREMONIAL_DRAW`) se difunden solo tras COMMIT efectivo, vía scope `AsyncLocalStorage` en `monitor-event-bus.js` integrado en `withTransaction` (`api/src/db/transaction.js`) y `runTransaction` (`api/src/modules/results/results-service.js`); `CLOSE_ATTEMPT_INCOMPLETE` conserva difusión inmediata (`immediate: true`) por ser alerta de anomalía.
- Spec 022: corrección post-commit el 2026-09-10: los eventos de dominio (`RESULTS_RELEASED`, `OFFICIAL_RECORD_EMITTED`, `RESULTS_TIE_BREAKER_CEREMONIAL_DRAW`) se difunden solo tras COMMIT efectivo, vía scope `AsyncLocalStorage` en `monitor-event-bus.js` integrado en `withTransaction` (`api/src/db/transaction.js`) y `runTransaction` (`api/src/modules/results/results-service.js`); `CLOSE_ATTEMPT_INCOMPLETE` conserva difusión inmediata (`immediate: true`) por ser alerta de anomalía.
- Spec 024: corrección el 2026-09-10: el canal SSE público solo notifica `RESULTS_SNAPSHOT_UPDATED` con payload `{eventId, version}` (`PUBLIC_NOTIFY_ACTIONS` en `api/src/routes/public.routes.js`); los eventos de dominio (`RESULTS_RELEASED`, `OFFICIAL_RECORD_EMITTED`) ya no se reenvían al portal.
...
- Spec 027: rediseño UX del panel administrador (2026-09-10). G0–G4 implementadas y validadas automáticamente: componentes admin, dashboard, configuración, competencia, operación/cierre, evento activo global con persistencia simple, separación Eventos/Competencia y catálogo visual de eventos. La validación manual responsive/teclado/táctil sigue pendiente.
- Spec 028: panel del jurado por jerarquía (AHORA/EVALUADAS/PRÓXIMAS) + planilla un-ítem-por-vez, cerrada el 2026-09-11. FIX-HOME/FIX-RAIL/FIX-SHELL del 2026-09-15 (RF-HOME-01–04): evaluada vs confirmada, revisión por planilla, rail sin desborde y alineación desktop; validación automatizada completa, comprobación manual pendiente. Ver `specs/028-panel-jurado-jerarquia/`.
