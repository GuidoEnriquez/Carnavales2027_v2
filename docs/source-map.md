...
- Spec 024: portal público de resultados (Fase 6 del Plan Maestro). Solo lectura post-liberación, snapshot inmutable `results_snapshot` (migración 070, triggers NO UPDATE/DELETE), materialización determinística JCS/SHA-256 en liberación/acta/sorteo, ETag/HTTP 304, SSE con fallback polling (30s) y vista `PublicResultsPage.jsx`. Validada y cerrada el 2026-09-08. Corrección el 2026-09-10: el canal SSE público solo notifica `RESULTS_SNAPSHOT_UPDATED` con payload `{eventId, version}` (`PUBLIC_NOTIFY_ACTIONS` en `api/src/routes/public.routes.js`); los eventos de dominio (`RESULTS_RELEASED`, `OFFICIAL_RECORD_EMITTED`) ya no se reenvían al portal.
