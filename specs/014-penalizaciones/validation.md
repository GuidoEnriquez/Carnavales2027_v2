...
| API | `npm test` en `api/` | 96 passed, 0 failed. Incluye suite `penalties-api.test.js` y `results-api.test.js` (autorización con/sin 2FA, segregación de liberación, ciclo de vida, ranking neto y preservación de premios individuales). |
| Cliente Tests | `npm test` en `client/` | 27 passed test files, 95 passed tests. Incluye `AdminPenaltiesPage.test.jsx`, `RevokePenaltyModal.test.jsx`, `AdminResultsPage.test.jsx`, `RequirePenaltiesRole.test.jsx`, `LoginPage.test.jsx`. |
| Cliente Build | `npm run build` en `client/` | Exitoso en 1.06s (Vite v7.3.6). |
...
## Comprobación manual realizada (390×844, 768×1024, 1440×900)

- [x] Carga de penalización en móvil con controles táctiles sin hover.
- [x] Listado accesible y contraste WCAG AA.
- [x] Diálogo de confirmación de revocación con teclado (Tab trap, Escape, retorno de foco).
- [x] Visualización clara del desglose de penalizaciones en la pantalla de resultados autorizados.
