...
- **RF-106 (recuperación del resultado).** CUANDO existe un `RESULTS_TIE_BREAKER_CEREMONIAL_DRAW` para el evento, EL SISTEMA DEBE permitir a un operador autorizado consultar su ganador, método, identificador de auditoría y fecha mediante `GET /api/v1/events/{eventId}/tie-breaker/ceremonial-draw`, sin crear, modificar ni repetir el sorteo. El cliente DEBE mostrar la ganadora en el modal si recibe `TIE_BREAKER_ALREADY_DRAWN` y en la vista de resultados al recargar. Si no existe sorteo, el endpoint responde `404 TIE_BREAKER_DRAW_NOT_FOUND`.
