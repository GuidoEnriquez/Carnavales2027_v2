...
4a. T09a (comparsas): cablear `brandColor` en servicio/rutas (sin migracion, columna 069 vigente), busqueda + filtros de vista, validacion inline por campo, preview "Vista jurado" de solo lectura.
4b. T09b (orden de pasada): `GET /events/:eventId/schedule?nightId=` (lectura ADMIN+2FA) y `POST /schedule/:scheduleId/reorder {direction, neighborId, expectedOrder, expectedNeighborOrder}` con intercambio transaccional y 409 obsoleto, clonando el contrato T08; UI Subir/Bajar por jornada, oculta con OPEN.
