...
* **Spec 028 FIX-HOME (2026-09-15):** el panel del jurado distingue comparsa evaluada de planilla confirmada (RF-HOME-01–04): AHORA solo con pendientes, completas en EVALUADAS con consulta, revisión por planilla completa, contador de evaluadas y rail desktop sin desborde. Validado automáticamente (suite cliente 327/327, build 88 módulos); comprobación manual en navegador pendiente.
* **Mesa de control (2026-09-15, Spec 003):** botón `Abrir jornada` (jornada `DRAFT` → `OPEN` vía endpoint existente) con indicador de estado, y `Abrir votación` deshabilitado hasta jornada `OPEN`. Sin endpoints ni reglas nuevas; el servidor sigue exigiendo evento `OPEN` → jornada `OPEN` → votación.
* **Correcciones menores UI (2026-09-15):** login con etiqueta `Correo electrónico` (el backend ya era solo email); corrección en Asignaciones del `ReferenceError` por `selectedNight` que rompía la página tras un reemplazo. Ambas con tests en verde.
