...
* **Spec 027 (jornadas con orden cronológico automático, validada automáticamente el 2026-09-14; comprobación manual responsive pendiente):** `display_order` es derivado de `event_date` (obligatoria). Crear/editar jornada reordena todo el evento por fecha; fechas duplicadas se rechazan (`NIGHT_DATE_DUPLICATE`, 409). Migración 071 (índice único parcial + renumeración). El cliente ya no envía orden manual.
* **Spec 028 (jerarquía de cabecera y orden automático de Tipos de participación y Especialidades, validada automáticamente el 2026-09-15; comprobación manual responsive pendiente):** `display_order` es automático al crear (`MAX+1` bajo advisory lock `category_order:{eventId}`/`specialty_order:{eventId}`); el reordenamiento posterior es exclusivo por Subir/Bajar (`POST /categories/:id/reorder` y `/specialties/:id/reorder`, swap transaccional con offset +1000000, concurrencia optimista `ORDER_CONFLICT`/`ORDER_BOUNDARY` 409, auditoría `CATEGORY_REORDERED`/`SPECIALTY_REORDERED`). La desactivación preserva huecos y una nueva alta recibe `MAX+1`. Creación/edición sin campo numérico de orden; cabecera verificada (eyebrow COMPETENCIA + `event.name` + Volver + pestañas). Sin migración nueva. Backend 149/149, cliente 261/261, build limpio.
